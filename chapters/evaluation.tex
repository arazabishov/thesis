...
\begin{itemize}    
    \item Add elements of two vectors.    
...
\subsubsection*{Add elements of two vectors}
\todo{}
% The benchmark has a strange name, it is not descriptive enough

% The test function is expected to add items at the same index from of integer vectors, and collect the results into a new vector. Two vectors are combined into a sequence of pairs of integers using the \emph{Zip} operator,

% The test function uses the \emph{Zip} combinator to combine two vectors into a single sequence of pairs, where
% The idea behind this benchmark is to add integers of two vectors at corresponding indices and to collect the results into a third vector. 

% The setup routine of this benchmark is identical for both tests, which generates a vector of [0, N] integers, where \emph{N} is the problem size. 

% The first test for incrementing numbers is implemented using the \emph{Map} combinator. It receives individual integers as argument, increments and returns the value back. 

% The second test relies on the \emph{Fold} combinator, which essentially is a predicate checking whether the number is odd or not. If it is odd, it keeps the value. Otherwise, it is filtered out. 

% As soon as processing is complete, parallel iterator is reduced to a vector. 
