...
Every core operation benchmark is parameterized over the vector size. By providing different arguments, we can observe how performance of vectors is affected in response. This is especially insightful for the tree based implementations, where the size of vector influences the height of the tree, which has a negative impact on performance. The output of a benchmark for a given size is the mean run time in \todo{ms}. 

\subsection{Indexing}

In this section we will define benchmarks for evaluating performance of accessing vector values, both sequentially and randomly. There are three use cases which will be tested:

\begin{itemize}
    \item Accessing an element at a given position by the index operation:
    \begin{itemize}
        \item Sequentially.
        \item Randomly.
    \end{itemize}
    \item Accessing elements sequentially using iterator. 
\end{itemize}

\subsubsection*{Index sequentially}

Accessing elements sequentially is a common pattern of reading values from a vector. 

The following benchmark is dedicated to understanding the performance properties of \rbtree{} and \rrbtree{} based vector implementations in comparison to the vector from the Rust's standard library. 
...
%  - What are we trying to understand in this benchmark.
%  - Does it depend on any preconditions? If yes, what are those preconditions and how they were satisfied? 
%  - What does the benchmark function does
