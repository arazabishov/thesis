...
% Results also depend on several factors such as the count of threads, count of CPU cores, as well as the problem size and work distribution mechanism. Benchmarks in this project have been parameterized to support various configurations. 

% All of the aforementioned benchmarks are following the map-reduce pattern \todo{ref}. When executed in parallel, work is split into several independent chunks of work, which at later stage are reduced to a result. 

% Rayon was inspired by Cilk \todo{ref}, the C programming language extension designed to simplify parallel programming at MIT. Scala, among other languages, also features parallel collection framework. 

% Rayon consists of three major components, which depend on each other in the following order:
% \begin{itemize}
...
% TODO: you should introduce the basic idea of iterators in the introduction
% TODO: provide a basic example of using par_iter

\subsection{Rayon}
% TODO: there is a fair amount of duplication between introduction and rayon sections
...
% TOOD: lazy evaluation of chains (when are they actually processed?)

\paragraph*{Processing stages}

% - parallel iterators is just a public api, doesn't perform actual split or load balancing
%  - Parallel iterator says I have a big pool of work to do, here are all the vectors and numbers and words
% - load balancing happens at the join or threadpool level (let's say rayon)

% how the work is divided between threads?
%  - what controls the count of threads?
%    - how is that relevant to the potential parallelism of the join framework?
%       - what is the join framework
%  - how to control the split factor, is it possible?
% how the results are combined back?
%  - again, join framework
%  - when it is necessary to concatenate the results
...
% TODO: static vs dynamic chunking
% TODO: convert to a code snippet
...
% Where does actual chunking is happening? I wouldn't be able to answer that question. 
% Actual chunking happens on the top level (?) 

% - How does Rayon work on primitive level?
    % - static chunking vs dynamic (?) chunking (or work stealing)
    ...
    % - how does rayon split work between threads         
    % - introduce split factor as mechanism to control chunking    
% - Configuring thread count in the underlying threadpool
% - Foundation of iter APIs: split / combine operations
    % - Types such as ParallelIterator
    % - Implementations for PVec and RrbVec

% - Computation stages of the fold benchmark:    
    % - splitting the work between P count of threads
    % - describes how the results are reduced or combined. 
    %     - Are results combined in particular order?
    %     - Fold and reduce operations
    % - using filter operator to force creating of new sequence
    %     - check if it is possible to create your own *faster* filter operator
