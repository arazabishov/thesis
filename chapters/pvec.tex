...
\begin{itemize}
    ...
    \item \crate{benches-mem}: The benchmarking crate for measuring the memory footprint of persistent vectors. Its usage is described in detail in \Cref{chapter:evaluation}.
\end{itemize}
...
The \pvecrs{} project can be configured using the feature flags described below:
\begin{table}[H]
...
All vectors provide methods with identical method signatures with the difference in how much each operation costs. The table below lists available functions and their complexity:
...
The complexity of operations is expressed using the Big \bigochar{} notation. The \bigo{1} value was substituted by $1$ for brevity.
...
The \pvecrs{} project draws inspiration from one of the Rust key design features --- zero-cost abstractions. In this section, we will look at how \pvec{} follows that design principle by offering optimizations such as transience and dynamic representation, without sacrificing Rust's safety guarantees.

\subsection{Transience as the language feature}
Popular persistent collection libraries in other languages, such as \emph{immutable-js}\footnote{\url{https://immutable-js.github.io/immutable-js/}} for JavaScript, or collections in the standard library of \emph{Scala}\footnote{\url{https://docs.scala-lang.org/overviews/parallel-collections/overview.html}}, often provide an interface that is different from the interface of the standard data structures.
...
Transience in Clojure\footnote{\url{https://clojure.org/reference/transients}} is not identical to the unique access optimization in Rust. These are the main differences:
...
\begin{itemize}
    ...
    \item Calling any method on a transient vector after it transitions back to persistent is prohibited and will cause an exception in Clojure. In Rust, there are no special restrictions imposed on vectors.
    \item To initialize a transient collection in Clojure, one has to use special syntax such as the \mintinline{clojure}{transient} function, or \mintinline{clojure}{persistent!} to convert a transient instance back to persistent.
\end{itemize}
...
If two vector types have identical interfaces, they can be used interchangeably without any additional changes made to the program. Hence, it becomes possible to dynamically choose a vector type depending on the context and performance requirements.
...
Standard vectors are very fast in almost all operations due to their efficient, hardware friendly design. Exceptions are operations that rely on copying, such as concat, split, and clone. Tree-based vectors, on the other hand, offers effectively \bigo{1} clones and good all-around performance.

To take the best of both worlds, a vector can start as standard vector and when it is cloned, it will be transformed into a tree-based one. Essentially, by switching the representation only when cloning, we offset the cost of using trees until they are beneficial\footnote{Nicholas Matsakis proposed a variation of this idea as future work in his blog: \url{http://smallcultfollowing.com/babysteps/blog/2018/02/01/in-rust-ordinary-vectors-are-values/}}. This optimization fits into the overall "Pay only for the features you use" idea, as it prevents the user from paying the abstraction cost until it is used.
...
\begin{figure}[!ht]
    ...
    \caption{Switching the vector representation}
