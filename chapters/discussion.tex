...
\paragraph*{Balanced vs relaxed variants}
Both variants perform well, with the balanced \rrbvec{} being faster by 1.36 in \range{[100, \mega{1}]} range. Both relaxed \rrbvec{} and \imrsvec{} are equally fast with insignificant marginal differences.

\paragraph*{Dynamic internal representation}
It is clear that \pvec{} is as fast as \stdvec{} up to the size of 1024, right after which it becomes slower and aligns with the \rrbvec{}. Even then, however, performance difference between \stdvec{} and \pvec{} is not significant, remaining quite consistent over the rest of the input range at 2.7. 
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}
        ...
        \pgfplotstableread[col sep=comma]{data/update/im-rs-vector-balanced.csv}\upimrsrbtree;
        \pgfplotstableread[col sep=comma]{data/update/im-rs-vector-unbalanced.csv}\upimrsrrbtree;
        \pgfplotstableread[col sep=comma]{data/update/pvec-balanced.csv}\uppvecrbtree;
        \pgfplotstableread[col sep=comma]{data/update/pvec-unbalanced.csv}\uppvecrrbtree;
        \pgfplotstableread[col sep=comma]{data/update/rrbvec-balanced.csv}\uprbvec;
        \pgfplotstableread[col sep=comma]{data/update/rrbvec-unbalanced.csv}\uprrbvec;
        \pgfplotstableread[col sep=comma]{data/update/std-vec.csv}\upstd;

        \pgfplotstableread[col sep=comma]{data/update_clone/im-rs-vector-balanced.csv}\upclimrsrbtree;
        \pgfplotstableread[col sep=comma]{data/update_clone/im-rs-vector-unbalanced.csv}\upclimrsrrbtree;
        \pgfplotstableread[col sep=comma]{data/update_clone/pvec-balanced.csv}\upclpvecrbtree;
        \pgfplotstableread[col sep=comma]{data/update_clone/pvec-unbalanced.csv}\upclpvecrrbtree;
        \pgfplotstableread[col sep=comma]{data/update_clone/rrbvec-balanced.csv}\upclrbvec;
        \pgfplotstableread[col sep=comma]{data/update_clone/rrbvec-unbalanced.csv}\upclrrbvec;
        \pgfplotstableread[col sep=comma]{data/update_clone/std-vec.csv}\upclstd;
