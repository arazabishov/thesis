...
% TODO: what about the main selling point? The cheap clone operation for vector. 
% TODO: lazy cloning: bootstrap this to unique access or rust's type system?
% TODO: this paper also contributes performance evaluation using 
%   reference counted pointers, rather than garbage collection
...
It is clear that the \rrbtree{}-based vectors perform better in the concatenation and slicing benchmarks. The difference in the performance compared to \rbtree{}-based vectors only increases as the vector size grows. 
...
% Mutating an RRB-Tree based implementation is shown to be more efficient when updated in-place without aliasing. Benchmarks for updating, pushing, popping, clearly show an improvement of TODOX percent over permutations of the same operations with clone used. 
...
% Along the performance benefits, the provided API implementation is identical to the one of standard vec. Hence, there is no need to learn a new API surface for developers who are already familiar with the std vector. TL/DR better ergonomics. 

% TODO: In parallel benchmarks, if vector is uniquely accessed, there is no need for cloning it at all. 
% TODO: specifics of when and by how much it is more efficient. 
...
% Was it effective?
%  * In which cases / scenarios? 
%    * Was it better than standard vector? 
%    * Was it better than rrbvec?
%    * How did it perform in parallel tests?
%  * Did it take advantage of the Rust's linear type system?
% Was it in-effective?
...
% The idea proposed in the \todo{scala paper} is to use standard vector 
...
% Dynamic internal representation showed improvements over RRB-Tree based vector implementation in most tests. The exceptions are tests that involved mutation and clone operation, and only up to the threshold value after which pvec switches from vec to rrbvec. Parallel vector implementation showed good results as well. 
...
% * PVec also contributes better performance in parallel tests, as it relies only on std vector representation in them
