...
\subsection{Adding elements of two vectors.} 
 ...
 - In sequential benchmark, both RbVec and RrbVec show identical performance results. This is due to the RrbTree being balanced in both cases, as the split and append operations are not used. 
 - std vector shows the best results. PVec is todox slower, while RbVec and RrbVec are the slowest with todox difference. 
 - With addition of parallelism, difference in performance between RbVec and RrbVec becomes noticeable at 2000 and up. This is due to a more efficient append / split implementation of RrbVec. The diff in performance becomes more apparent between pvec and stdvec at 2000 (that's when cache misses start happening?)
  * When does pvec upgrade from stdvec? Specifically, when using split / append operations? 
 - The difference between rbvec and rrbvec becomes even more significant with addition of more threads. Again, this is connected to the fact that append / split are faster for rrbvec. The interesting point here is that perf difference between std / pvec becomes less noticeable.
  * Again, when does pvec upgrade / downgrade? Is it connected to the ref count? 
 - What is interesting, is that parallel implementation is slower than a sequential one. This can be explained by the fact that operation which we perform in the benchmark is very fast. Thus, single thread performance wins over parallel implementation due to the overhead of parallelisation. 
...
\rbvec{} and \rrbvec{} produce identical results in the single thread test. This is expected, because sequential benchmark does not use concatenation or splitting, the operations which create unbalanced nodes. 

The difference quickly becomes apparent with parallelisation of work on 2 and 4 threads. \rbvec{} demonstrates TODOX slower results compared to \rrbvec{}, due to the naive concat and split operations.

\paragraph*{Dynamic internal representation}

Performance for 2, 4 cores is different compared to 8. Probably because of internal representation, which changes as well as more splitting / appending which happens in between. 

% * pvec and dynamic internal representation

\paragraph*{Effect of parallelism}
% * benefits of parallelism
%   * single thread performance
%   * correlation of perf improvements with thread count increase

In the test with 8 threads, the difference becomes even more noticeable at the factor of TODOX. As the number of available threads increases, Rayon breaks down the work into smaller chunks to keep all threads busy.

This can be explained by the fact that with increased count of threads, Rayon breaks down the work into smaller chunks, which implies increased use of the concat and split operations. 

Performance in fact degenerates, for all vector implementations at 8 threads. Provide a possible explanation. 
