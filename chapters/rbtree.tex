...
\begin{listing}[ht!]    
    ...
    \begin{algorithmic}
        \Function{RadixSearch}{root, height, key}
            \State $node \leftarrow root$

            \For{$level \leftarrow height - 1, 1$}
...
To summarize, the update operation performs the \h count copies of \m sized nodes, where \h is the maximum height of the tree. As described in section \ref{sec:rb-tree-memory-layout}, \h is bound by \bigo{log_m(n)}, which results in the {\bigo{m * log_m(n)}} complexity of the update operation. 
...
\begin{listing}[ht!]    
    ...
    \begin{algorithmic}
        \Function{Update}{root, height, key, value}
