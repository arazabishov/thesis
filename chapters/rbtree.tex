...
\begin{listing}[ht!]
    ...
    \begin{algorithmic}
        ...
        \State newTail $\leftarrow$ Clone(vec\textsubscript{tail})
        \State newRoot $\leftarrow$ NIL

        \State value $\leftarrow$ newTail[newTail\textsubscript{size} - 1]
        \State newTail\textsubscript{size} $\leftarrow$ newTail\textsubscript{size} - 1     
            
        \If{newTail\textsubscript{size} = 0}
            \State newRoot, newTail $\leftarrow$ RbTree-Pop(vec\textsubscript{root})            
        \Else 
            \State newRoot $\leftarrow$ vec\textsubscript{root}
        \EndIf
...
\begin{listing}[ht!]
    ...
    \begin{algorithmic}
        ...
        \State root $\leftarrow$ vec\textsubscript{root}
        \State tail $\leftarrow$ vec\textsubscript{tail}

        \If{key < root\textsubscript{size}}
            \State newRoot $\leftarrow$ RbTree-Update(root, key, value)
            \State \Return CreateVec(newRoot, tail)
        \Else
            \State newTail $\leftarrow$ Clone(tail)
            \State newTail[key - root\textsubscript{size}] $\leftarrow$ value
...
\begin{listing}[ht!]
    ...
    \begin{algorithmic}
        ...
        \Else            
            \State \Return tail[key - root\textsubscript{size}]            
        \EndIf
        \EndFunction
    \end{algorithmic}
\end{listing}