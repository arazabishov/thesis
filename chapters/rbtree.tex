...
Since values are residing both in \rbtree\ and tail, one has to check where the given search key is located. A value is located within the tree if the key is less than the tree size. In this case the search process is delegated to \rbtree. Otherwise, the index for value in the tail is calculated by by subtracting the tree size from the key. 
